...
- (void)requestWithMethod:(RequestMethod)method  url:(NSString*)url dict:(NSDictionary*)dict hasHeader:(BOOL)hadHeader finished:(Finished)finished failed:(Failed)failed {
    InstensiveRequest *request = [[InstensiveRequest alloc]init];
    request.requestMethod = method;
    request.url = url;
    request.paramDic = dict;
    request.finished = finished;
    request.failed = failed;
    request.isAutoMask = YES;
    request.requestType = Ordinary;
    request.hasHeader = hadHeader;
    [request beginRequest];
}
