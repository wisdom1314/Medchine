...
+ (MBProgressHUD *)createDefaultHUD{
    // 取消之前的loading
    while([MBProgressHUD HUDForView:[self getCurrentWindow]] != nil){
        [MBProgressHUD hideHUDForView:[self getCurrentWindow] animated:NO];
    }
...
+ (void)dismiss{
    ...
    MBProgressHUD *hud = [MBProgressHUD HUDForView:[self getCurrentWindow]];
    if (hud) {
        [hud hideAnimated:YES];
    }
}
