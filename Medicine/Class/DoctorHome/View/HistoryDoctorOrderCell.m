...
@interface HistoryDoctorOrderCell()
...
@implementation HistoryDoctorOrderCell
...
+ (instancetype)getTableView:(UITableView *)tableView indexPathWith:(NSIndexPath *)indexPath {
    ...
    HistoryDoctorOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HistoryDoctorOrderCellId"];
    if(!cell) {
        cell = [[[NSBundle mainBundle]loadNibNamed:@"HistoryDoctorOrderCell" owner:self options:nil]objectAtIndex:0];
    }
    return cell;
}
